...
begin
	...
	using Climatology, MITgcm, CSV, JLD2, NetCDF
...
function plot(𝐼::Individuals)
	...
	plt=contourf!(ax,𝐵.lon,𝐵.lat,permutedims(𝐵.fld),
		levels = range(𝐵.rng..., length = 10), colormap = :ice)

	🔴_by_t = groupby(𝐼.🔴, :t)
	lo=deepcopy(🔴_by_t[1].lon); lo[findall(lo.<xlims[1])]=lo[findall(lo.<xlims[1])].+360
	scatter!(ax,lo,🔴_by_t[1].lat,markersize=12,color=:red,marker = :circle)
	lo=deepcopy(🔴_by_t[end].lon); lo[findall(lo.<xlims[1])]=lo[findall(lo.<xlims[1])].+360
	scatter!(ax,lo,🔴_by_t[end].lat,markersize=12,color=:black,marker = :star5)
...
manifest_format = "2.0"
project_hash = "cbb970b8bbcc734fc8f0abe2174636f7b2305197"
...
git-tree-sha1 = "8c5785ae96aa589abb0135d95107e26ec68a13a2"
...
git-tree-sha1 = "243a8a229daaa040d3966f2e3e076c25ce142289"
